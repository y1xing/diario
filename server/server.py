...
# Contact me for the key.json file
...
def get_diaries_emotion_summary(uid):
    ...
    if diary_entries_data is not None:
        ...
        for entry in diary_entries:
            ...
            if emotions is not None:
                ...
                for item in emotions:
                    ...
                    if diaries_emotion_summary.get(item.get('emotion').lower()) is None:
                        print(item.get('emotion'))
                        diaries_emotion_summary[item.get(
                            'emotion').lower()] = 1
                    else:
                        diaries_emotion_summary[item.get(
                            'emotion').lower()] += 1
        ...
        return jsonify(
            diary_count=entry_counter,
            emotion_summary=diaries_emotion_summary,

        )

    return json.dumps({"error": ERROR_DIARY_DATA_NOT_FOUND})
...
def post_diary():
    ...
    #                         'and fondness (5%).', {'Friends': 1, 'Family': 0}]
    ...
    return json.dumps({"analysisResult": aspects_sentiment_formatted})
