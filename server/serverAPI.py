...
class GetDiariesEmotionSummary(Resource):
    def get(self, uid):
        entry_counter = 0
...
class PostDiary(Resource):
    def post(self, diary_entry):

        # Split argument
        uid, diary_title, diary_content = diary_entry.split('+')
        ...
        # TODO Run content through analysis
        analysis_results = ['The emotions expressed in the sentence are likely to be happiness (80%), nostalgia (15%), '
                            'and fondness (5%).', {'Friends': 1, 'Family': 0, 'Life': -1, 'Life': -1}]
        ...
        for key, value in analysis_results[1].items():
            aspect = key.lower()
...
class GetAnalysisSummary(Resource):
    def get(self, diary_date_range):

        # Split arguments
        uid, date_start, date_end = diary_date_range.split('+')
        ...
        if diary_entries_data is not None:
            ...
            for entry in diary_entries_data:
                ...
                if datetime.datetime.strptime(date_start, '%Y-%m-%d') <= date <= datetime.datetime.strptime(date_end, '%Y-%m-%d'):
                    diary_entries.append(diary)
            ...
            for key, value in sentiment_over_time.items():
                ...
                for date, score in value.items():
                    if date in repeated_dates.keys():
                        value[date] = round(
                            (score / repeated_dates.get(date)), 2)
...
api.add_resource(GetDiariesEmotionSummary, '/diariesEmotionSummary/<uid>')
api.add_resource(PostDiary, '/writeDiary/<diary_entry>')
api.add_resource(GetAnalysisSummary, '/analysisSummary/<uid>')
